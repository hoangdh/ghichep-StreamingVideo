...
# Lay thu muc DocumentRoot
#grep -i 'DocumentRoot' /etc/httpd/conf/httpd.conf|sed '/^#/ d' | awk {'print $2'} | sed -r 's/"//g'
...
if [ -z "$list" ]; then
	for x in {5..1}
		do
			clear
			echo -e "(c) MediTech JSC,.. & LongVan IDC\nKhong co file trong thu muc, script se tu dong thoat sau $x."
			sleep 1
		done
	clear
	exit
